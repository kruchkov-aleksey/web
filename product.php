...
$id_product=strip_tags(mysqli_real_escape_string($connect, $_GET['id']));

$result_product=mysqli_query($connect, "SELECT baza.id, baza.name, baza.author, baza.price, baza.file, baza.inf, years.year FROM baza JOIN years ON years.id=baza.id WHERE baza.id='$id_product' ");
$result_product=mysqli_fetch_assoc($result_product);
...
<head>
    ...
    <title><?=$result_product['name']?></title>
    <!-- MDB icon -->
...
<main>
    ...
    <img alt='oops' title='Москва-Петушки' class='knigaimg' src='<?=$result_product['file']?>'>




    <div class='knigainfo'>
        <p class='knigainfo1'><?=$result_product['name']?></p><br>
        <p class='knigainfo1'><?=$result_product['author']?></p><br>
        <p class='knigainfo1'><?=$result_product['inf']?></div>
    <br>
    <p class='kupit'><a class='catalhov' href='ukrali.php'>КУПИТЬ</a></p>
    <p class='kupit'><?=$result_product['price']?>р.</p>
